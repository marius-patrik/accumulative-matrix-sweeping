...
def test_committed_routed_expert_comparison_rejects_threshold_only_ternary() -> None:
    ...
    assert comparison["decision_status"] == "residual2_requires_layer_quality_validation"
    ...
    residual = variants["residual2-ternary-threshold-08-of-10"]
    assert residual["selected_tensor_encoded_bytes"] == 283_115_520
    assert residual["cosine_similarity"] == 0.9805015653892943
    assert residual["normalized_root_mean_square_error"] == 0.20045466131651093
    assert residual["selected_tensor_encoded_bytes"] < int4["selected_tensor_encoded_bytes"]
    assert (
        residual["normalized_root_mean_square_error"]
        < best_ternary["normalized_root_mean_square_error"] / 2
    )
